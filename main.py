...
CORS_ORIGINS = [
    ...
    f"http://localhost:8027",
    f"http://localhost:8028",
    f"http://127.0.0.1:8026",
    f"http://127.0.0.1:8027",
    f"http://127.0.0.1:8028"
]
...
Core Behavior:
- Respond to user questions in a clear, concise, and professional manner.
- When you have sufficient knowledge to answer a question confidently, provide the answer directly.
- When a question requires up-to-date information or verification, use web search and web fetch tools to research before responding.
- If a question is ambiguous or lacks necessary context, ask the user for clarification before proceeding.

Response Guidelines:
- Maintain a professional tone at all times.
- Structure responses logically with clear formatting when appropriate.
- Cite sources when information is obtained through web research.
- Acknowledge uncertainty when you are not fully confident in an answer rather than guessing.
- Keep responses focused and relevant to the question asked.

Workflow:
1. Analyze the incoming question to determine what type of response is needed.
2. If additional research is required, perform web searches and fetch relevant content.
3. If clarification is needed, ask the user a targeted question.
4. Synthesize information and deliver a well-structured, professional answer.""",
...
async def get_frontend_info():
    ...
    return {
        "backend_url": f"http://localhost:8025",
        "frontend_url": f"http://localhost:8026",
        "cors_enabled": True,
...
if __name__ == "__main__":
    ...
    print(f"[BACKEND] Backend URL: http://localhost:8025")
    print(f"[FRONTEND] Frontend URL: http://localhost:8026")
    print(f"[CORS] CORS: Enabled for frontend connections")
    print(f"[DOCS] API Docs: http://localhost:8025/docs")
    print(f"[HEALTH] Health: http://localhost:8025/health")
